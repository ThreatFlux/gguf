...
fn test_large_tensor_handling() {
    ...
    assert!(write_result.total_bytes_written > size as u64 * 4);
}
...
fn test_many_small_tensors() {
    ...
    assert!(!bytes.is_empty());
    assert_eq!(write_result.tensor_count, 100);
}
...
fn test_large_metadata() {
    ...
    assert!(!bytes.is_empty());
    assert!(write_result.metadata_bytes_written > 10000); // Should be substantial
}