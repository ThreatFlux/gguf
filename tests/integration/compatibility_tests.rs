...
fn test_empty_file_compatibility() {
    ...
    assert_eq!(write_result.tensor_count, 0);
    assert_eq!(write_result.metadata_count, 0);
    
    // Read it back
    let cursor = Cursor::new(bytes);
    let reader = gguf::reader::GGUFFileReader::new(cursor).expect("Failed to read empty file");
...
fn test_minimal_valid_file() {
    ...
    assert!(!bytes.is_empty());
    assert_eq!(write_result.tensor_count, 1);
    assert_eq!(write_result.metadata_count, 1);
    
    // Verify it can be read
    let cursor = Cursor::new(bytes);
    let reader = gguf::reader::GGUFFileReader::new(cursor).expect("Failed to read minimal file");
...
fn test_special_characters_in_names() {
    ...
    let data = vec![0u8; 4];
    let result = builder
        .add_tensor("tensor.with.dots", vec![1], TensorType::F32, data.clone()).unwrap()
        .add_tensor("tensor-with-dashes", vec![1], TensorType::F32, data.clone()).unwrap()
        .add_tensor("tensor_with_underscores", vec![1], TensorType::F32, data).unwrap();
...
fn test_zero_dimensional_edge_cases() {
    ...
    let builder = result.unwrap();
    let (bytes, write_result) = builder.build_to_bytes().expect("Failed to build with empty tensor");
    ...
    let cursor = Cursor::new(bytes);
    let reader = gguf::reader::GGUFFileReader::new(cursor).expect("Failed to read empty tensor");
...
fn test_unicode_string_handling() {
    ...
    builder = builder
        ...
        .add_metadata("mixed", MetadataValue::String("Mixed: ASCII + 中文 + العربية + 🌍".to_string()));
