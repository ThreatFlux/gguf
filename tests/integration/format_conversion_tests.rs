...
fn test_version_compatibility() {
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build v3 file");
...
fn test_endianness_handling() {
    ...
    let loaded_data = reader.load_tensor_data("endian_test")
        .expect("Failed to load data")
        .expect("Data should exist");
    
    let loaded_values: Vec<f32> = loaded_data.as_slice()
        .chunks_exact(4)
...
fn test_alignment_handling() {
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build with alignment");
    ...
    for i in 1..=4 {
        ...
        let tensor_info = reader.get_tensor_info(&tensor_name).unwrap();
        
        // Verify the offset is properly aligned
        // Note: In the actual implementation, offsets should be aligned to DEFAULT_ALIGNMENT
        assert_eq!(tensor_info.offset() % 32, 0, "Tensor {} offset {} is not aligned to 32 bytes", i, tensor_info.offset());
        
        // Verify we can load the data
        let data = reader.load_tensor_data(&tensor_name)
            .expect("Failed to load tensor")
            .expect("Tensor should exist");
        
        assert_eq!(data.len(), tensor_info.size_in_bytes() as usize);
    }
}
...
fn test_string_encoding() {
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build with strings");
    
    let cursor = Cursor::new(bytes);
    let reader = GGUFFileReader::new(cursor).expect("Failed to read strings");
    
    for (key, expected_value) in &test_strings {
        let actual_value = reader.metadata().get_string(key)
...
fn test_large_metadata() {
    ...
    for i in 0..1000 {
        builder = builder.add_metadata(
            ...
            MetadataValue::String(format!("Large metadata value number {} with some padding text to make it longer", i))
        );
    }
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build large metadata");
    ...
    for i in [0, 100, 500, 999] {
        ...
        let expected = format!("Large metadata value number {} with some padding text to make it longer", i);
        assert_eq!(reader.metadata().get_string(&key), Some(&expected));
    }
...
fn test_tensor_name_edge_cases() {
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build with various names");
    
    let cursor = Cursor::new(bytes);
    let mut reader = GGUFFileReader::new(cursor).expect("Failed to read various names");
    
    assert_eq!(reader.tensor_count() as usize, test_names.len());
    
    for (i, name) in test_names.iter().enumerate() {
        let tensor_info = reader.get_tensor_info(name)
        ...
        let values: Vec<f32> = data.as_slice()
            .chunks_exact(4)
            .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            .collect();
...
fn test_zero_sized_tensors() {
    ...
    let bytes = builder.build_to_bytes().expect("Failed to build with empty tensors");
    ...
    let normal_data = reader.load_tensor_data("normal")
...
fn test_file_size_calculation() {
    ...
    assert_eq!(result.tensor_data_bytes_written as usize, expected_tensor_data);
    assert!(result.header_bytes_written > 0);
    assert!(result.metadata_bytes_written > 0);
    assert!(result.tensor_info_bytes_written > 0);
    
    let calculated_total = result.header_bytes_written + 
    ...
    assert_eq!(file_size, result.total_bytes_written);
}