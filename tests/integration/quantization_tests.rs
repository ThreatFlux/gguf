...
fn test_quantization_basic() {
    // Verify representative legacy and K-quant storage geometry.
...
fn test_quantization_size_calculation() {
    ...
    assert_eq!(q4_size, Some(18)); // 32 4-bit values + 2 bytes metadata per block

    let q8_size = TensorType::Q8_0.calculate_size(32); // Size for one block (32 elements)
    assert_eq!(q8_size, Some(34)); // 32 8-bit values + 2 bytes metadata per block
}
