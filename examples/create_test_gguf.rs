...
fn main() -> Result<()> {
    ...
    metadata.insert("general.description".to_string(), MetadataValue::String("A simple test model for gguf_rs".to_string()));
    ...
    let vocab_tensor = TensorInfo::new("token_embd.weight".to_string(), vocab_shape, TensorType::F32, 0);
    ...
    let norm_tensor = TensorInfo::new("output_norm.weight".to_string(), norm_shape, TensorType::F32, 0);
